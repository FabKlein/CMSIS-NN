 ...
 * Title:        arm_conv_specialized_f32.h
...
#ifndef ARM_CONV_SPECIALIZED_F32_H
#define ARM_CONV_SPECIALIZED_F32_H
...
static inline void arm_conv_execute_1d_f32(arm_conv_1d_kernel_f32 kernel,
                                           ...
                                           float32_t *output)
{
    for (int32_t y = 0; y < output_dims->h; ++y)
    {
        ...
        float32_t *output_row = output + (size_t)y * output_dims->w * output_dims->c;
        if (input_y >= 0 && input_y < input_dims->h)
        {
            const float32_t *input_row = input + (size_t)input_y * input_dims->w * input_dims->c;
...
static inline void arm_conv_execute_specialized_f32(const arm_conv_selection_f32 *selection,
                                                    const cmsis_nn_conv_params_f32 *params,
                                                    const cmsis_nn_dims *input_dims,
...
{
    const arm_conv_kernels_f32 kernels = arm_conv_resolve_specialized_f32(selection);
...
#endif /* ARM_CONV_SPECIALIZED_F32_H */
