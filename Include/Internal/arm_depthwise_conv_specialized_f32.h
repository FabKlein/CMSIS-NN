 ...
 * Title:        arm_depthwise_conv_specialized_f32.h
...
#ifndef ARM_DEPTHWISE_CONV_SPECIALIZED_F32_H
#define ARM_DEPTHWISE_CONV_SPECIALIZED_F32_H
...
typedef arm_cmsis_nn_status (*arm_dw_call_f32)(const cmsis_nn_dw_conv_params_f32 *params,
...
static arm_cmsis_nn_status arm_dw_spec_k9_1d_nhwc_f32_call(const cmsis_nn_dw_conv_params_f32 *params,
                                                           const cmsis_nn_dims *input_dims,
                                                           const float32_t *input,
                                                           const float32_t *kernel,
                                                           const float32_t *bias,
                                                           ...
                                                           float32_t *output)
{
    ...
    const size_t input_batch_stride = (size_t)input_dims->h * input_w * input_c;
    const size_t output_batch_stride = (size_t)output_dims->h * output_w * output_c;

    for (int32_t b = 0; b < input_dims->n; ++b)
    {
        const float32_t *input_b = input + (size_t)b * input_batch_stride;
        float32_t *output_b = output + (size_t)b * output_batch_stride;

        for (int32_t y = 0; y < output_dims->h; ++y)
        {
            arm_nn_depthwise_conv1d_k9_nhwc_f32(input_b + (size_t)y * input_w * input_c,
                                                input_c,
                                                input_w,
                                                params->ch_mult,
                                                kernel,
                                                bias,
                                                output_b + (size_t)y * output_w * output_c,
                                                output_w);
        }
    }
...
static arm_cmsis_nn_status arm_dw_spec_k3_1d_nhwc_f32_call(const cmsis_nn_dw_conv_params_f32 *params,
                                                           const cmsis_nn_dims *input_dims,
                                                           const float32_t *input,
                                                           const float32_t *kernel,
                                                           const float32_t *bias,
                                                           ...
                                                           float32_t *output)
{
    ...
    for (int32_t b = 0; b < batch; ++b)
    {
        ...
        for (int32_t y = 0; y < output_h; ++y)
        {
            ...
            arm_nn_depthwise_conv1d_k3_nhwc_f32(input_row, input_c, input_w, kernel, bias, output_row, output_w);
        }
    }
...
static arm_cmsis_nn_status arm_dw_spec_2x5_nhwc_f32_call(const cmsis_nn_dw_conv_params_f32 *params,
                                                         const cmsis_nn_dims *input_dims,
                                                         const float32_t *input,
                                                         const float32_t *kernel,
                                                         const float32_t *bias,
                                                         ...
                                                         float32_t *output)
{
    arm_nn_depthwise_conv2x5_nhwc_f32(input,
                                      input_dims->n,
                                      input_dims->c,
                                      input_dims->w,
                                      params->ch_mult,
...
static arm_cmsis_nn_status arm_dw_spec_3x3_nhwc_f32_call(const cmsis_nn_dw_conv_params_f32 *params,
                                                         const cmsis_nn_dims *input_dims,
                                                         const float32_t *input,
                                                         const float32_t *kernel,
                                                         const float32_t *bias,
...
static const arm_dw_call_f32 arm_dw_calls_nhwc_f32[] = {
...
static inline arm_dw_call_f32 arm_dw_select_nhwc_f32(const cmsis_nn_dw_conv_params_f32 *params,
                                                     const cmsis_nn_dims *input_dims,
...
{
    size_t index;
    if (arm_depthwise_conv_select_specialized_f32(params, input_dims, filter_dims, output_dims, kernel_layout, &index))
    {
        return arm_dw_calls_nhwc_f32[index];
    }
    return NULL;
}
#endif

#endif /* ARM_DEPTHWISE_CONV_SPECIALIZED_F32_H */
