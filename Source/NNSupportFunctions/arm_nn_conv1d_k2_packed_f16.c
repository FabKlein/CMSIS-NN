 ...
 * Title:        arm_nn_conv1d_k2_packed_f16.c
 * Description:  Support: NHWC 1D convolution kernel size 2 for packed f16 weights
 *
                                 ...
                                 int32_t out_w)
{
    ...
    for (; ox + 5 < out_w; ox += 6)
    {
        float16_t *dst0 = out + ((size_t)(ox + 0)) * (size_t)out_c;
        float16_t *dst1 = out + ((size_t)(ox + 1)) * (size_t)out_c;
        float16_t *dst2 = out + ((size_t)(ox + 2)) * (size_t)out_c;
        float16_t *dst3 = out + ((size_t)(ox + 3)) * (size_t)out_c;
        float16_t *dst4 = out + ((size_t)(ox + 4)) * (size_t)out_c;
        float16_t *dst5 = out + ((size_t)(ox + 5)) * (size_t)out_c;
        ...
        for (; oc + block_cols <= out_c; oc += block_cols)
        {
            ...
            {
                const float16_t *row0 = x_nhwc + ((size_t)(ox + 0)) * (size_t)in_c;
        ...
        if (oc < out_c)
        {
            ...
            for (int32_t ky = 0; ky < kernel_h; ++ky)
            {
                ...
                for (int32_t kx = 0; kx < kernel_w; ++kx)
                {
                    ...
                    for (int32_t ic = 0; ic < in_c; ++ic)
                    {
                        const float16x8_t w = vld1q_z(w_base + (kernel_col_base + (size_t)ic) * (size_t)block_cols, p);
                        acc0 = vfmaq(acc0, w, x0[ic]);
                        acc1 = vfmaq(acc1, w, x1[ic]);
                        acc2 = vfmaq(acc2, w, x2[ic]);
                        acc3 = vfmaq(acc3, w, x3[ic]);
    ...
    for (; ox < out_w; ++ox)
    {
        float16_t *dst = out + ((size_t)ox) * (size_t)out_c;
        int32_t oc = 0;
        for (; oc + block_cols <= out_c; oc += block_cols)
        {
            const float16_t *w_base = kernel_packed + ((size_t)oc / block_cols) * kernel_plane * block_cols;
            ...
            for (int32_t ky = 0; ky < kernel_h; ++ky)
            {
                const float16_t *row = x_nhwc + ((size_t)ox) * (size_t)in_c;
                for (int32_t kx = 0; kx < kernel_w; ++kx)
                {
                    const size_t kernel_col_base = ((size_t)ky * (size_t)kernel_w + (size_t)kx) * (size_t)in_c;
