                                                      ...
                                                      arm_nn_weight_format_flt weight_format)
{
    ...
    if (weight_format != ARM_NN_WEIGHT_FORMAT_NT_N_PACKED &&
        (patch_len < ARM_NN_CONV_NHWC_PATCH_GEMM_F16_MIN_K || output_c < ARM_NN_CONV_NHWC_PATCH_GEMM_F16_MIN_OC ||
                                          ...
                                          float16_t *output_data)
{
    ...
    if (arm_conv_select_specialized_f16(conv_params, input_dims, filter_dims, output_dims, &selection))
    {
        arm_conv_execute_specialized_f16(
    ...
    const bool use_patch_gemm =
        arm_conv_nhwc_use_patch_gemm_f16(ctx, patch_len, output_c, output_positions, conv_params->weight_format);
