...
#include "Internal/arm_conv_specialized_f16.h"
...
#include "../TestData/conv_k5_batch2_rows2_tails_null_bias_f16/test_data.h"
...
#include "../TestData/conv_k9_exact_six_f16/test_data.h"
...
void conv_k3_opt_f16_arm_convolve_f16_packed(void)
{
    ...
    const int32_t buf_size =
        arm_convolve_f16_get_buffer_size(&conv_params, &input_dims, &filter_dims, &output_dims, CONV_K3_OPT_F16_LAYOUT);
    if (buf_size > 0)
    {
        ctx.buf = malloc((size_t)buf_size);
        ctx.size = buf_size;
...
void conv_k5_opt_f16_arm_convolve_f16_packed(void)
{
    ...
    const int32_t buf_size =
        arm_convolve_f16_get_buffer_size(&conv_params, &input_dims, &filter_dims, &output_dims, CONV_K5_OPT_F16_LAYOUT);
    if (buf_size > 0)
    {
        ctx.buf = malloc((size_t)buf_size);
        ctx.size = buf_size;
...
void conv_k5_output_width_boundaries_f16(void)
{
    enum
    {
        BATCHES = 2,
    ...
    static float16_t input[MAX_INPUT_SIZE];
    static float16_t weights[WEIGHT_SIZE];
    static float16_t output_standard[MAX_OUTPUT_SIZE];
    static float16_t output_packed[MAX_OUTPUT_SIZE];
    static float16_t reference[MAX_OUTPUT_SIZE];

    for (int32_t i = 0; i < WEIGHT_SIZE; ++i)
    {
        weights[i] = (float16_t)(((i % 7) - 3) * 0.0625f);
    }
    float16_t *packed_weights = pack_rhs_nt_n_from_nt_t_f16(weights, OUT_CH, KERNEL_W * IN_CH);
    ...
    for (int32_t out_w = 1; out_w <= MAX_OUT_W; ++out_w)
    {
        ...
        {
            input[i] = (float16_t)(((i % 11) - 5) * 0.03125f);
        }
                    ...
                    for (int32_t oc = 0; oc < OUT_CH; ++oc)
                    {
                        ...
                        for (int32_t kw = 0; kw < KERNEL_W; ++kw)
                        {
                            for (int32_t ic = 0; ic < IN_CH; ++ic)
                            {
                                ...
                                sum += (float32_t)input[input_index] * (float32_t)weights[weight_index];
                            }
                        }
                        sum = sum > 0.125f ? 0.125f : sum;
                        sum = sum < -0.125f ? -0.125f : sum;
                        reference[(((n * ROWS + h) * out_w + ow) * OUT_CH) + oc] = (float16_t)sum;
                    }
                }
            }
        }
        ...
        cmsis_nn_conv_params_f16 params = {.padding = {0, 0},
                                           .stride = {1, 1},
                                           .dilation = {1, 1},
                                           .activation = {(float16_t)-0.125f, (float16_t)0.125f},
                                           .weight_format = ARM_NN_WEIGHT_FORMAT_STANDARD};

        int32_t buf_size =
            arm_convolve_f16_get_buffer_size(&params, &input_dims, &filter_dims, &output_dims, ARM_NN_LAYOUT_NHWC);
        cmsis_nn_context ctx = {.buf = buf_size > 0 ? malloc((size_t)buf_size) : NULL, .size = buf_size};
        arm_cmsis_nn_status status = arm_convolve_f16(&ctx,
                                                      &params,
                                                      &input_dims,
                                                      input,
                                                      &filter_dims,
        ...
        {
            TEST_ASSERT_FLOAT_WITHIN(2.0e-2f, (float)reference[i], (float)output_standard[i]);
            TEST_ASSERT_FLOAT_WITHIN(2.0e-2f, (float)reference[i], (float)output_packed[i]);
        }
    }
...
#define RUN_CONV_F16_PACKED_CASE(CASE_PREFIX, case_name, bias_data, tolerance)                                         \
...
RUN_CONV_F16_PACKED_CASE(CONV_K5_BATCH2_ROWS2_TAILS_NULL_BIAS_F16,
                         conv_k5_batch2_rows2_tails_null_bias_f16,
                         NULL,
                         2.0e-2f)
RUN_CONV_F16_PACKED_CASE(CONV_K7_OPT_F16, conv_k7_opt_f16, conv_k7_opt_f16_biases_data, 2.0e-2f)
RUN_CONV_F16_PACKED_CASE(CONV_K9_EXACT_SIX_F16, conv_k9_exact_six_f16, conv_k9_exact_six_f16_biases_data, 2.0e-2f)
RUN_CONV_F16_PACKED_CASE(CONV_K9_OPT_F16, conv_k9_opt_f16, conv_k9_opt_f16_biases_data, 2.0e-2f)
RUN_CONV_F16_PACKED_CASE(CONV_K2_PACKED_VALID_F16,
                         conv_k2_packed_valid_f16,
                         conv_k2_packed_valid_f16_biases_data,
                         2.0e-2f)
RUN_CONV_F16_PACKED_CASE(CONV_KERNEL_2X2_F16, conv_kernel_2x2_f16, conv_kernel_2x2_f16_biases_data, 2.0e-2f)
RUN_CONV_F16_PACKED_CASE(CONV_2X3_PACKED_VALID_F16,
                         conv_2x3_packed_valid_f16,
                         conv_2x3_packed_valid_f16_biases_data,
                         2.0e-2f)
RUN_CONV_F16_PACKED_CASE(CONV_2X5_PACKED_VALID_F16,
                         conv_2x5_packed_valid_f16,
                         conv_2x5_packed_valid_f16_biases_data,
                         2.0e-2f)
 ...
 */
static void check_packed_conv2d_tiles_f16(const int32_t kernel_w)
{
    enum
    {
        IN_CH = 3,
    ...
    const int32_t channels[] = {1, 8, MAX_OUT_CH};
    static float16_t input[IN_H * MAX_IN_W * IN_CH];
    static float16_t weights[MAX_OUT_CH * MAX_PATCH_LEN];
    static float16_t bias[MAX_OUT_CH];
    static float16_t guarded_output[OUT_H * MAX_OUT_W * MAX_OUT_CH + 2];
    float16_t *output = guarded_output + 1;

    for (int32_t i = 0; i < IN_H * MAX_IN_W * IN_CH; ++i)
    {
        input[i] = (float16_t)((i % 17 - 8) * 0.0625f);
    }
    for (int32_t i = 0; i < MAX_OUT_CH * MAX_PATCH_LEN; ++i)
    {
        weights[i] = (float16_t)((i % 7 - 3) * 0.03125f);
    }
    for (int32_t i = 0; i < MAX_OUT_CH; ++i)
    {
        bias[i] = (float16_t)((i % 3 - 1) * 0.125f);
    }

    for (unsigned int c = 0; c < sizeof(channels) / sizeof(channels[0]); ++c)
    {
        const int32_t out_ch = channels[c];
        float16_t *packed = pack_rhs_nt_n_from_nt_t_f16(weights, out_ch, patch_len);
...
void conv_2x2_packed_tiles_f16(void) { check_packed_conv2d_tiles_f16(2); }
void conv_2x3_packed_tiles_f16(void) { check_packed_conv2d_tiles_f16(3); }
void conv_2x5_packed_tiles_f16(void) { check_packed_conv2d_tiles_f16(5); }

static void check_packed_conv_cancellation_f16(const int32_t kernel_w)
{
    enum
    {
        IN_CH = 2,
    ...
    static float16_t input[MAX_IN_W * IN_CH];
    static float16_t weights[MAX_OUT_CH * MAX_KERNEL_W * IN_CH];
    static float16_t output[MAX_OUT_W * MAX_OUT_CH];
    const int32_t channel_counts[] = {1, 8, 9};
    ...
    for (unsigned int channels = 0; channels < sizeof(channel_counts) / sizeof(channel_counts[0]); ++channels)
    {
        ...
        float16_t *packed_weights = pack_rhs_nt_n_from_nt_t_f16(weights, out_ch, patch_len);
...
void conv_k7_packed_cancellation_f16(void) { check_packed_conv_cancellation_f16(7); }
void conv_k9_packed_cancellation_f16(void) { check_packed_conv_cancellation_f16(9); }

/* Exercise every registration through the public API, including future entries. */
void conv_specialized_registry_f16(void)
{
    ...
    #include "Internal/arm_conv_specialized_registry_f16.h"
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i)
    {
        ...
        float16_t *input = malloc((size_t)input_count * sizeof(*input));
        float16_t *weights = malloc((size_t)patch_len * out_c * sizeof(*weights));
        float16_t *output = malloc((size_t)output_count * sizeof(*output));
        ...
        for (int32_t oc = 0; oc < out_c; ++oc)
        {
            ...
            {
                weights[oc * patch_len + k] = (float16_t)(oc + 1);
            }
        }
        float16_t *packed_weights = pack_rhs_nt_n_from_nt_t_f16(weights, out_c, patch_len);
        for (int32_t packed = 0; packed <= 1; ++packed)
        {
            ...
            cmsis_nn_conv_params_f16 params = {.stride = spec->stride,
            ...
            TEST_ASSERT_EQUAL_MESSAGE(
                available,
                arm_conv_select_specialized_f16(&params, &input_dims, &filter_dims, &output_dims, &selection),
                message);
            TEST_ASSERT_EQUAL_MESSAGE(
                available,
                arm_conv_select_specialized_f16(&params, &input_dims, &filter_dims, &output_dims, NULL),
                message);
            if (!available)
            {
                continue;
            ...
            for (int32_t property = 0; property < 3; ++property)
            {
                ...
                {
                    cmsis_nn_conv_params_f16 changed = params;
                    ...
                    TEST_ASSERT_TRUE(arm_conv_specialized_nhwc_geometry_is_valid(&changed.stride,
                                                                                 ...
                                                                                 &filter_dims,
                                                                                 &changed_output));
                    TEST_ASSERT_FALSE(arm_conv_select_specialized_f16(
                        &changed, &changed_input, &filter_dims, &changed_output, &selection));
                    TEST_ASSERT_FALSE(
                        arm_conv_select_specialized_f16(&changed, &changed_input, &filter_dims, &changed_output, NULL));
            ...
            ++wrong_output.w;
            TEST_ASSERT_FALSE(arm_conv_select_specialized_f16(&params, &input_dims, &filter_dims, &wrong_output, NULL));
            params.weight_format = (arm_nn_weight_format_flt)99;
            TEST_ASSERT_FALSE(arm_conv_select_specialized_f16(&params, &input_dims, &filter_dims, &output_dims, NULL));
        }
        free(packed_weights);
        free(output);
        free(weights);
    ...
    TEST_ASSERT_FALSE(arm_conv_select_specialized_f16(&params, &input_dims, &filter_dims, &output_dims, NULL));
    TEST_ASSERT_TRUE(
        arm_convolve_f16_get_buffer_size(&params, &input_dims, &filter_dims, &output_dims, ARM_NN_LAYOUT_NHWC) > 0);
#endif
}

void conv_specialized_geometry_f16(void)
{
    static const struct
    {
        int32_t input;
...
static void conv_test_stride2_f16(const float16_t *input,
                                  int32_t input_c,
                                  int32_t input_w,
                                  ...
                                  int32_t output_w)
{
    ...
    for (int32_t x = 0; x < output_w; ++x)
    {
        for (int32_t oc = 0; oc < output_c; ++oc)
        {
            ...
            output[x * output_c + oc] = (float16_t)acc;
        }
    }
}
#endif

void conv_specialized_stride2_adapter_f16(void)
{
#ifdef NN_DISABLE_SPECIALIZATION
    TEST_IGNORE_MESSAGE("Specialized kernels are disabled");
#else
    ...
    for (int32_t with_bias = 0; with_bias <= 1; ++with_bias)
    {
        ...
        arm_conv_execute_1d_f16(
            conv_test_stride2_f16, &params, &input_dims, input, weights, with_bias ? bias : NULL, &output_dims, output);
        for (size_t i = 0; i < ARM_CONV_ARRAY_SIZE(output); ++i)
        {
