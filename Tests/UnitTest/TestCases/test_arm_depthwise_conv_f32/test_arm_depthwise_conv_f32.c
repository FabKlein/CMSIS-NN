...
#include "../TestData/depthwise_k2_1d_opt_nhwc_f32/test_data.h"
#include "../TestData/depthwise_k3_1d_opt_batch2_f32/test_data.h"
#include "../TestData/depthwise_k3_1d_opt_nhwc_f32/test_data.h"
#include "../TestData/depthwise_k5_1d_opt_nhwc_f32/test_data.h"
#include "../TestData/depthwise_k7_1d_opt_nhwc_f32/test_data.h"
#include "../TestData/depthwise_k9_1d_mult3_batch2_null_bias_f32/test_data.h"
#include "../TestData/depthwise_k9_1d_mult9_f32/test_data.h"
#include "../TestData/depthwise_k9_1d_opt_nhwc_f32/test_data.h"
...
RUN_DEPTHWISE_F32_CASE(DEPTHWISE_K2_1D_OPT_NHWC_F32, depthwise_k2_1d_opt_nhwc_f32, 5.0e-4f)
RUN_DEPTHWISE_F32_CASE(DEPTHWISE_K5_1D_OPT_NHWC_F32, depthwise_k5_1d_opt_nhwc_f32, 5.0e-4f)
RUN_DEPTHWISE_F32_CASE(DEPTHWISE_K7_1D_OPT_NHWC_F32, depthwise_k7_1d_opt_nhwc_f32, 5.0e-4f)
RUN_DEPTHWISE_F32_CASE(DEPTHWISE_K9_1D_OPT_NHWC_F32, depthwise_k9_1d_opt_nhwc_f32, 5.0e-4f)
RUN_DEPTHWISE_F32_CASE(DEPTHWISE_K9_1D_MULT3_BATCH2_NULL_BIAS_F32, depthwise_k9_1d_mult3_batch2_null_bias_f32, 5.0e-4f)
RUN_DEPTHWISE_F32_CASE(DEPTHWISE_K9_1D_MULT9_F32, depthwise_k9_1d_mult9_f32, 5.0e-4f)
...
void depthwise_invalid_geometry_f32(void)
{
    const float32_t input[1] = {2};
    const float32_t kernel[9] = {3};
    float32_t output[9];
    ...
    for (unsigned int i = 0; i < sizeof(invalid) / sizeof(invalid[0]); ++i)
    {
        ...
        *invalid[i].field = invalid[i].value;
        for (int route = 0; route < 3; ++route)
        {
            ...
            if (route == 0)
            {
                status = arm_depthwise_conv_f32(NULL,
                                                &params,
                                                &input_dims,
                                                input,
                                                &filter_dims,
...
void depthwise_geometry_limits_f32(void)
{
    const float32_t input[1] = {2};
    const float32_t kernel[9] = {3};
    float32_t output[4] = {17, 17, 17, 17};
    const cmsis_nn_dims input_dims = {.n = 1, .h = 1, .w = 1, .c = 1};
    cmsis_nn_dims filter_dims = {.n = 1, .h = 3, .w = 3, .c = 1};
    cmsis_nn_dims output_dims = {.n = 1, .h = 2, .w = 2, .c = 1};
    cmsis_nn_dw_conv_params_f32 params = {.ch_mult = 1,
...
void depthwise_dilated_clipping_boundaries_f32(void)
{
    const float32_t input[1] = {2};
    const float32_t kernel[1] = {3};
...
void depthwise_specialization_selection_f32(void)
{
    static const struct
    {
        const char *name;
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i)
    {
        ...
        TEST_ASSERT_EQUAL_MESSAGE(cases[i].specialized,
                                  arm_dw_select_nhwc_f32(&params, &in, &filter, &out, ARM_NN_DW_KERNEL_KC) != NULL,
                                  cases[i].name);
        TEST_ASSERT_TRUE_MESSAGE(arm_dw_select_nhwc_f32(&params, &in, &filter, &out, ARM_NN_DW_KERNEL_CK) == NULL,
                                 cases[i].name);
        ++out.w;
        TEST_ASSERT_TRUE_MESSAGE(arm_dw_select_nhwc_f32(&params, &in, &filter, &out, ARM_NN_DW_KERNEL_KC) == NULL,
                                 cases[i].name);
        --out.w;
#endif
        for (int32_t j = 0; j < in.n * in.h * in.w * in.c; ++j)
        {
            input[j] = (float32_t)(j % 7 - 3);
        }
        for (int32_t j = 0; j < filter.h * filter.w * out.c; ++j)
        {
            kernel[j] = (float32_t)(j % 5 - 2);
        }
        for (int32_t j = 0; j < out.n * out.h * out.w * out.c; ++j)
        {
            output[j] = (float32_t)999;
        }
...
void depthwise_specialization_requirements_f32(void)
{
    static const struct
    {
        const char *name;
 ...
 */
void depthwise_small_multiplier_vectors_f32(void)
{
    ...
    static float32_t input[BATCHES * INPUT_H * MAX_IN_W * MAX_IN_C];
    static float32_t kernel[10 * MAX_OUT_C];
    static float32_t bias[MAX_OUT_C];
    static float32_t output_storage[BATCHES * INPUT_H * MAX_OUT_W * MAX_OUT_C + 2];
    float32_t *output = output_storage + 1;
        ...
        for (size_t c = 0; c < sizeof(channel_counts) / sizeof(channel_counts[0]); ++c)
        {
            ...
            for (size_t m = 0; m < sizeof(multipliers) / sizeof(multipliers[0]); ++m)
            {
                ...
                for (size_t w = 0; w < sizeof(output_widths) / sizeof(output_widths[0]); ++w)
                {
                    ...
                    {
                        input[i] = (float32_t)((i % 7 - 3) * 0.125f);
                    }
                    for (int32_t i = 0; i < kernel_h * kernel_w * out_c; ++i)
                    {
                        kernel[i] = (float32_t)((i % 5 - 2) * 0.0625f);
                    }
                    for (int32_t i = 0; i < out_c; ++i)
                    {
                        bias[i] = (float32_t)((i % 3 - 1) * 0.5f);
                    }
                    for (int32_t with_bias = 0; with_bias < 2; ++with_bias)
                    {
                        ...
                        {
                            output_storage[i] = (float32_t)99;
                        }
                        TEST_ASSERT_EQUAL_MESSAGE(ARM_CMSIS_NN_SUCCESS,
                                                  arm_depthwise_nhwc_conv_f32(NULL,
                                                                              &params,
                                                                              &input_dims,
                                                                              input,
                                                                              &filter_dims,
...
void depthwise_specialized_scratch_f32(void)
{
    static const struct
    {
        const char *name;
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i)
    {
        ...
        const int32_t size = arm_depthwise_conv_f32_get_buffer_size(&params, in, &filter, &out, ARM_NN_LAYOUT_NHWC);
        TEST_ASSERT_EQUAL_INT_MESSAGE(
            size, arm_depthwise_conv_wrapper_f32_get_buffer_size(&params, in, &filter, &out), cases[i].name);
        ...
        else if (params.ch_mult == 1)
        {
            ...
            TEST_ASSERT_EQUAL_INT_MESSAGE(
                4 * filter.h * filter.w * in->c * (int32_t)sizeof(float32_t), size, cases[i].name);
        }
        else
        {
        ...
        TEST_ASSERT_EQUAL_MESSAGE(
            ARM_CMSIS_NN_SUCCESS,
            arm_depthwise_conv_wrapper_f32(NULL, &params, in, input, &filter, kernel, NULL, NULL, &out, output),
            cases[i].name);
        for (int32_t j = 0; j < output_count; ++j)
        {
    ...
    params.padding.w = INT32_MAX;
    TEST_ASSERT_GREATER_THAN_INT(0, arm_depthwise_conv_wrapper_f32_get_buffer_size(&params, &in, &filter, &out));
    params.padding.w = 0;
    params.stride.w = 0;
    TEST_ASSERT_GREATER_THAN_INT(0, arm_depthwise_conv_wrapper_f32_get_buffer_size(&params, &in, &filter, &out));
    params.stride.w = 1;
    ...
    out.w = 2;
    TEST_ASSERT_GREATER_THAN_INT(0, arm_depthwise_conv_wrapper_f32_get_buffer_size(&params, &in, &filter, &out));
    TEST_ASSERT_EQUAL_INT(0, arm_depthwise_conv_wrapper_f32_get_buffer_size(NULL, &in, &filter, &out));
}
